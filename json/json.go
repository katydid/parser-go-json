...
import (
	...
	"github.com/katydid/parser-go-json/json/fastfloat"
...
func (s *jsonParser) Double() (float64, error) {
	if s.isLeaf {
		i, err := fastfloat.ParseFloat(s.buf)
		return i, err
	}
...
func (s *jsonParser) Int() (int64, error) {
	if s.isLeaf {
		i, err := fastfloat.ParseInt64(s.buf)
		if err != nil {
			f, ferr := fastfloat.ParseFloat(s.buf)
			if ferr != nil {
				return i, err
			}
...
func (s *jsonParser) Uint() (uint64, error) {
	if s.isLeaf {
		i, err := fastfloat.ParseUint64(s.buf)
		return uint64(i), err
	}
